...
public class Stepdefs {
    ...
    public void when_username_and_password_are_entered(String username, String password) throws Throwable {
        inputLines1.add(username);
        inputLines1.add(password);

        io = new StubIO(inputLines1);
        app = new App(io, auth);
        app.run();
    }
    
    @Then("^Then system will respond with \"([^\"]*)\"$")
    public void then_system_will_respond_with(String expectedOutput) throws Throwable {
        assertTrue(io.getPrints().contains(expectedOutput));
    }
